...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📊 Referral tracked:', { referralCode, email: email.toLowerCase(), tier, timestamp: Date.now() });
    return NextResponse.json({
      success: true,
      referralCode,
      status: tier === 'free' ? 'pending' : 'converted',
      message: tier === 'free' ? 'Referral tracked. Will convert when user upgrades.' : 'Referral converted!',
    });
  } catch {
